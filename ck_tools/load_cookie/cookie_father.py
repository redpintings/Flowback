...
def load(domain_name: list) -> dict:
    """Try to load cookies from all supported browsers and return combined cookiejar
    Optionally pass in a domain name to only load cookies from the specified domain
    """
    item = dict()
    cj_ = [http.cookiejar.CookieJar() for dom in domain_name]
    for cookie_fn in [chrome, edge, firefox, safari]:
        try:
            for domain, _cj_ in zip(domain_name, cj_):
                for cookie in cookie_fn(domain_name=domain):
                    _cj_.set_cookie(cookie)
                item[domain] = _cj_
